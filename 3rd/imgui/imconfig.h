...
namespace ImGui
{
	...
	bool ComboAlt(const char* data_ex, const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items = -1);
	void SetItemDefaultFocusAlt();
	void SetItemFocusAlt(const char* item_id, bool force_focus = false);
	bool IsItemFocusedAlt(const char* item_id);
	bool IsItemActiveAlt(const char* item_id);
	void EraseColumn(const char* item_id, int columns_count);
}

#ifdef IMGUI_USER_USE_TLS_FOR_GIMGUI
#define GImGui ((ImGuiContext*)ImGui::GetGImGui())
#define IMGUI_SET_CURRENT_CONTEXT_FUNC ImGui::SetGImGui
#endif
